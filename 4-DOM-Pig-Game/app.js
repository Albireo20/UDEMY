...
*/
var scores, roundScore, activePlayer, diceDOM, gamePlaying, previousDiceRoll, diceRoll;
	...
	if (gamePlaying) {
		...
		if (dice !== 1) {
				...
				if (previousDiceRoll == 6 && diceRoll == 6) {
					...
					//console.log('Previous Dice Roll 2: ' + previousDiceRoll);
				}
		} else {
				nextActivePlayer();
		}
...
function nextActivePlayer() {
	//Next Player
		activePlayer === 0 ? activePlayer = 1 : activePlayer = 0;  //Turnary Operator
		roundScore = 0;
		diceRoll = 0;
		previousDiceRoll = 0;
...
1. A player looses his ENTIRE score when he rolles two 6 in a row. After that, it's the next player's turn. (Hint: Always save the previous dice roll in a separate variable).
